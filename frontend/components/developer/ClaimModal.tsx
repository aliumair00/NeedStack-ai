...
export default function ClaimModal({ card, onConfirm, onClose }: ClaimModalProps) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-md bg-[#111118] border border-white/10 rounded-2xl overflow-hidden">
        {/* Header */}
        ...
        <div className="px-5 py-5">
          ...
          <div className="mb-5">
            ...
            <textarea
              ...
              rows={3}
              maxLength={300}
              className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2.5 text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-indigo-500/50 transition-colors resize-none"
            />
            <div className="text-right text-[10px] text-slate-600 mt-1">{note.length}/300</div>
          </div>

          {/* Info box */}
          <div className="bg-indigo-500/5 border border-indigo-500/20 rounded-xl p-3 mb-5">
